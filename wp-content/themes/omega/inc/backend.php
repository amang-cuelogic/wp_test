...
function oxy_create_logo_css() {
    ...
    $header_sass = oxy_file_get_contents( 'assets/scss/_variables.scss' );
    $header_sass .= oxy_file_get_contents( 'assets/scss/_compass-mixins.scss' );
    $header_sass .= oxy_file_get_contents( 'assets/scss/_mixins.scss' );
...
function oxy_compile_swatch_scss( $post_id, $force_file_save = false ) {
    // get swatch mixins & variables
    $swatch_sass = oxy_file_get_contents( 'assets/scss/_variables.scss' );
    $swatch_sass .= oxy_file_get_contents( 'assets/scss/_compass-mixins.scss' );
    $swatch_sass .= oxy_file_get_contents( 'assets/scss/_mixins.scss' );
    ...
    if( get_post_meta( $swatch->ID, THEME_SHORT . '_status', true ) === 'enabled' ) {
        ...
        if( oxy_get_option( 'swatch_css_save' ) === 'file' || $force_file_save ) {
...
add_action( 'wp_ajax_nopriv_install_default_swatches', 'oxy_install_default_swatches_ajax' );
...
function oxy_install_default_swatches() {
    ...
    foreach( $default_swatches as $class => $swatch ) {
        ...
        oxy_compile_swatch_scss( $new_swatch_id, true );
    }
...
function oxy_create_default_colour_css() {
    // get swatch mixins & variables
    $default_sass = oxy_file_get_contents( 'assets/scss/_variables.scss' );
    $default_sass .= oxy_file_get_contents( 'assets/scss/_compass-mixins.scss' );
    $default_sass .= oxy_file_get_contents( 'assets/scss/_mixins.scss' );
...
function oxy_enqueue_ajax_save_menu( $current_screen ) {
    if ( 'nav-menus' === $current_screen->base && oxy_get_option( 'ajax_menu_save' ) === 'on' ) {
        wp_enqueue_script( 'oxy-ajax-save-menu', OXY_THEME_URI . '/inc/assets/js/ajax-save-menu.js', array( 'jquery' ) );
    }
}
add_action( 'current_screen', 'oxy_enqueue_ajax_save_menu' );
...
add_action('wp_ajax_oxy_save_menu', 'oxy_save_menu');